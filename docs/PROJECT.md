...
It hosts a local HTTP endpoint for wall displays. Milestone 4 adds a **Widget Engine** and an official Homey **App Settings Dashboard Editor**: per-device widget configuration (Device Store), explicit placement without overlaps, and the first two real widgets (`TitleWidget`, `DateTimeWidget`).

## Current status

**Milestone 4 is implemented.** Milestone 0–3 behavior is preserved (HTTP server, settings, drivers, registry, recognition, grid engine, diagnostics).
...
| Diagnostics page | Done (M2/M3/M4) |
| Vanilla grid rendering from device layout | Done (M3) |
| Widget Registry + Title / DateTime widgets | Done (M4) |
| Dashboard Editor in App Settings | Done (M4) |
| Homey capability controls | Not started |
...
4. Reuse `lib/display`, `lib/dashboard`, `lib/widgets`, `lib/adapters`, `lib/pairing`, `lib/http`. Do not invent a second persistence layer for displays or layouts.
...
| Path | Role |
| --- | --- |
| `app.ts` | Homey App lifecycle; HTTP + DisplayRegistry + editor API |
| `api.ts` | Homey Web API for Dashboard Editor |
| `lib/widgets/` | Shared widget types, placement, validation, registry |
...
| `frontend/widgets/` | Isolated widget renderers (title, date-time) |
